...
int main() {
    ...
    Monostable q_button;
    Monostable l_mouse_button;
    Monostable r_mouse_button;
    Camera<float> camera;
    ...
    while (!window.exitRequested()) {
        ...
        l_mouse_button.update(input.getButton(0));
        r_mouse_button.update(input.getButton(1));
        ...
        scene.update(center, q, player_position, player.getFacing(), *vc.get(), l_mouse_button.state(), r_mouse_button.state());
